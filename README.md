...
pip install -r djangobackend/requirements.txt
```
...
The Django API deploys to [Render](https://render.com/) using the Blueprint in [`render.yaml`](render.yaml), which creates:

- **`calendar-db`**: a free Render Postgres database (free databases expire after 30 days)
- **`calendar-api`**: a free web service that builds from `djangobackend/` with [`build.sh`](djangobackend/build.sh) and runs with gunicorn

On every deploy, `build.sh` installs dependencies, collects static files, runs migrations, and creates the superuser from `DJANGO_SUPERUSER_EMAIL` and `DJANGO_SUPERUSER_PASSWORD` if it doesn't exist yet.

Settings are read from environment variables, so the same code runs locally and on Render:

| Variable | Local default | On Render |
| --- | --- | --- |
| `DATABASE_URL` | SQLite (`djangobackend/db.sqlite3`) | Render Postgres connection string |
| `SECRET_KEY` | Insecure development key | Randomly generated by Render |
| `DEBUG` | `True` | `False` |
| `RENDER_EXTERNAL_HOSTNAME` | Not set | Set by Render; added to `ALLOWED_HOSTS` |

Free web services spin down when idle, so the first request after a while can take up to a minute.
